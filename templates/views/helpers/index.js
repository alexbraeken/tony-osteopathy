...
module.exports = function () {
	...
	// Used to generate the link for the admin edit article button
	...
	_helpers.articleUrl = function (articleSlug, options) {
		return ('/articles/article/' + articleSlug);
	};
	...
	/*
	* expecting the data.articless context or an object literal that has `previous` and `next` properties
	...
	* */
	_helpers.ifHasPagination = function (articleContext, options) {
		...
		if (_.isUndefined(articleContext)) {
			return options.inverse(this);
		}
		if (articleContext.next || articleContext.previous) {
			return options.fn(this);
		}
		return options.inverse(this);
	};
